...
#define baseline_sift
//#define performance_sift

#ifdef performance_sift
    F2D* sift(F2D* I)
    {
        int rows, cols, K;
        ...
        **/
        ...
        sigma0 = 1.6f * pow(2, (1.0f/subLevels));
        sigman = 0.5f;
        thresh = (0.04f/subLevels)/2;
        r = 10;
            ...
            
            {
                ...
                for(i=0; i<idx->height; i++)
                {
                    for(j=0; j<idx->width; j++)
                    {
                        ...
                        subsref(y,i,j) = ceil((z / sizeRows)+0.5f);

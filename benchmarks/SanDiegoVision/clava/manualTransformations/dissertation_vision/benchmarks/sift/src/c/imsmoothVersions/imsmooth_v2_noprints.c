    ...
    if(s > threshold)
    {
      ...
      **/
      ...
      for(j = 0 ; j < M; ++j)
        for(i = 0 ; i < W; ++i)
          for(k=0; k<=i+W; k++)
              subsref(buffer,j,i) += subsref(array, j, k) * temp[k+W-i];
      //Janela deslizante
      for(j = 0 ; j < M ; ++j)
        for(i = W ; i < N-W-1; ++i)
          for(k=0; k<=2*W; k++)
              subsref(buffer,j,i) += subsref(array, j, k+i-W) * temp[k];
      //Moldura direita
      for(j = 0 ; j < M ; ++j)
        for(i = N-W-1 ; i < N ; ++i)
          for(k=0; k<=N+W-1-i; k++)
      ...
      **/
      ...
      for(j = 0; j < W; ++j)
        for(i = 0; i < N; ++i)
          for(k=0; k<=j+W; k++)
              subsref(out,j,i) += subsref(buffer,k,i) * temp[k+W-j]; //filterstart++ <=> k+W+j
      //Valores interiores da matriz
      for(j = W; j < M-W-1; ++j)
        for(i = 0; i < N; ++i)
          for(k=0; k<=2*W; k++)
              subsref(out,j,i) += subsref(buffer,k+j-W,i) * temp[k];
      //Moldura inferior
      for(j = M-W-1; j < M; ++j)
        for(i = 0; i < N; ++i)
          for(k=0; k<=M+W-j-1; k++)

   ...
   for(o = 0; o < Octaves; o++) {
         ...
         for(i1 = 0; i1 < (intervals - 1); i1++) {
            for(j = 0; j < sizeCols; j++) {
               for(i = 0; i < sizeRows; i++) {
                  temp->data[k] = dogss[o * intervals + i1]->data[(i) * dogss[o * intervals + i1]->width + (j)];
                  negate->data[k++] = -dogss[o * intervals + i1]->data[(i) * dogss[o * intervals + i1]->width + (j)];
               }
            }
         }
      }
      ...
      **/
      oframes = fiDeepCopy(i_);
      {
         ...
         **/
         if(discardBoundaryPoints) oframes = filterBoundaryPoints(sizeRows, sizeCols, temp);
         fFreeHandle(temp);
      }
      /**
      ...
      **/
      if(oframes->data[0] != 0) {
         ...
         else frames = fDeepCopy(oframes);
         firstIn = 0;
      }
      else if(Octaves == 1) frames = fDeepCopy(oframes);
      fFreeHandle(oframes);
      iFreeHandle(y);
      iFreeHandle(x);
      iFreeHandle(s_);

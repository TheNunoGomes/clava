...
F2D * siftlocalmax(F2D *in, float thresh, int intervals, int M, int N) {
   ...
   **/
   ndims = 3;
   /*Since we have intervals number of images of size M x N*/
   dims[0] = M;
   dims[1] = N;
   dims[2] = intervals - 1;
   /*
   ...
   */
   for(h = 0; h < ndims; ++h) if(dims[h] < 3) goto end;
   while(1) {
      /*Propagate carry along multi index midx*/
      h = 0;
      while(midx[h] >= dims[h] - 1) {
         midx[h] = 1;
         if(++h >= ndims) goto next_layer;
         ++midx[h];
      }
      /*Scan neighbors*/
      {
         ...
         ((ptoffset < pt->width * pt->height) ? (void) (0) : __assert_fail("ptoffset < pt->width*pt->height", "/tmp/__clava_woven_a8ae5b44-8dfd-4ef9-ab9b-a247830a8182_osboxes/siftlocalmax.c", 174, __extension__ "F2D *siftlocalmax(F2D *, float, int, int, int)"));
         i = 0;
   ...
   /*Return.*/
   {
      ...
      for(i = 0; i < maxIter; i++) out->data[i] = maxima_iterator[i];
   }
   /*Release space.*/
   free(neighbors);
   free(maxima_start);
